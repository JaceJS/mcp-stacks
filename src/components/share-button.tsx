...
export function ShareButton({ slug, title }: { slug: string; title: string }) {
  ...
  const url = `https://mcpstacks.dev/stacks/${slug}`;
  ...
  function handleShareTwitter() {
    const text = encodeURIComponent(`Check out "${title}" on MCP Stacks — a curated MCP server combo:\n${url}`);
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-[var(--border)] text-[13px] text-[var(--foreground-muted)] hover:border-[var(--border-hover)] hover:text-[var(--foreground)] transition-all cursor-pointer bg-transparent"
      >
